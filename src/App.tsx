...
// Messaging component
function MessagingPanel({ currentUser, otherUser, messages, onSendMessage, onClose }: {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ flex: 1, padding: 16, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 8, background: '#f9fafb' }}>
          {conversationMessages.map(message => (
            ...
            >
              <div style={{
                ...
                padding: '10px 14px',
                borderRadius: 18,
                background: message.senderEmail === currentUser.email ? '#3b82f6' : '#e5e7eb',
                color: message.senderEmail === currentUser.email ? 'white' : '#1f2937',
                border: message.flagged ? '2px solid #ef4444' : 'none',
                wordWrap: 'break-word'
              }}>
                <div style={{ fontSize: 11, opacity: 0.7, marginBottom: 4 }}>
                  {message.senderName} • {new Date(message.createdAt).toLocaleTimeString()}
                  {message.flagged && <span style={{ color: '#fca5a5', marginLeft: 8 }}>🚨 FLAGGED</span>}
                </div>
                <div style={{ fontSize: 14, lineHeight: 1.4 }}>{message.content}</div>
                {message.flagged && (
                  <div style={{ fontSize: 10, color: '#fca5a5', marginTop: 4, fontStyle: 'italic' }}>
                    This message was flagged for suspicious content and reported to admin
                  </div>
                )}
              </div>
            </div>
...
// Toast notification component for new message alerts
function ToastNotification({ message, onClose }: { message: string; onClose: () => void }) {
  ...
  return (
    <div style={{
      ...
      <div>
        <div style={{ fontWeight: 600, fontSize: 14 }}>New Message</div>
...
// Determine fee: 10% commission for platform, 90% goes to designer
function determineFeeKES(budgetStr?: string) {
  const budget = Number(String(budgetStr || '').replace(/[^\d.]/g, ''))
  if (!isFinite(budget) || budget <= 0) return 600 // Default minimum fee

  // Your 10% commission
  const commission = Math.round(budget * 0.1)

  // Minimum commission of 400 KES, maximum of 800 KES
  return Math.max(400, Math.min(800, commission))
}

// Calculate designer's earnings (90% of budget)
function calculateDesignerEarnings(budgetStr?: string) {
  const budget = Number(String(budgetStr || '').replace(/[^\d.]/g, ''))
  ...
  return (
    <aside className="sidebar" style={{ display: 'grid', gap: 8 }}>
      ...
      <div style={{ fontSize: 12, opacity: 0.8, marginTop: 8 }}>
        Payments: KES 400–800 per request, based on budget
      </div>
      {!isAdmin && (
        <button
        ...
        <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: 8 }}>
          {recentRequests.map((r) => (
            <li key={r.id} style={{ border: '1px solid var(--border)', borderRadius: 8, padding: 8 }}>
              <strong>{r.candidateName}</strong> — {r.office}
              <div style={{ fontSize: 12, opacity: 0.8 }}>Budget: {r.budget || 'N/A'} • Deadline: {r.deadline || 'N/A'}</div>
            </li>
          ))}
        </ul>
      </aside>
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16, zIndex: 1000 }}>
      <div className="checkout-panel" style={{ background: 'var(--bg-elev)', width: 'min(560px, 96vw)', borderRadius: 12, padding: 16, boxShadow: '0 10px 30px rgba(0,0,0,0.2)' }}>
        ...
        <div style={{ marginBottom: 12, fontSize: 14 }}>
          <div><strong>Posting: </strong>{request.candidateName} — {request.office}</div>
        ...
        <div style={{ border: '1px solid var(--border)', borderRadius: 8, padding: 12, marginBottom: 12 }}>
          ...
          <div style={{ fontSize: 12, opacity: 0.8, marginTop: 4 }}>
            You pay {formatKES(fee)} (10% commission) and the designer receives 90% of the project budget.
            {Number(String(request.budget || '').replace(/[^\d.]/g, '')) > 0 &&
              ` Designer will earn ${formatKES(calculateDesignerEarnings(request.budget))}`
            }
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
          ...
          <button onClick={simulatePay} className="button" disabled={processing}>{processing ? 'Processing…' : `Simulate Payment ${formatKES(fee)}`}</button>
        </div>
      </div>
    </div>
  )
...
function ClientDashboard({ requests, userEmail, onAddReview, onSendMessage, messages }: { requests: DesignRequest[]; userEmail: string; onAddReview: (review: Omit<Review, 'id' | 'createdAt'>) => void; onSendMessage: (message: Omit<Message, 'id' | 'createdAt' | 'read'>) => void; messages: Message[] }) {
  ...
  const [newMessageToast, setNewMessageToast] = useState<{ id: string; message: string } | null>(null)

  // Real-time message filtering - this will update automatically when messages prop changes
  const getConversationMessages = (requestId: string) => {
    const request = requests.find(r => r.id === requestId)
    if (!request || !request.acceptedDesignerId) return []
    return messages.filter(m =>
      (m.senderEmail === userEmail && m.receiverEmail === request.acceptedDesignerId) ||
      (m.senderEmail === request.acceptedDesignerId && m.receiverEmail === userEmail)
    )
  }

  // Detect new messages and show toast notifications
  useEffect(() => {
    const unreadMessages = messages.filter(m =>
      m.receiverEmail === userEmail && !m.read && m.senderEmail !== userEmail
    )

    if (unreadMessages.length > 0) {
      const latestMessage = unreadMessages[0] // Get the most recent unread message
      const request = requests.find(r => r.id === latestMessage.requestId)

      if (request && !newMessageToast) {
        setNewMessageToast({
          id: latestMessage.id,
          message: `New message from ${latestMessage.senderName} about "${request.candidateName} - ${request.office}"`
        })
      }
    }
  }, [messages, userEmail, requests, newMessageToast])
  ...
  function handleAddReview(reviewData: Omit<Review, 'id' | 'createdAt'>) {
    onAddReview({
      ...
      reviewerName: userEmail.split('@')[0]
    })
    setShowReviewForm(null)
  }

  function handleSendMessage(content: string, designerEmail: string) {
    onSendMessage({
      requestId: '',
    ...
    <div className="client-dashboard" style={{ display: 'grid', gap: 16 }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 12 }}>
        <div style={{ border: '2px solid #f59e0b', borderRadius: 12, padding: 12, background: '#fef3c7' }}>
          <div style={{ fontSize: 12, opacity: 0.8, color: '#92400e' }}>💳 Payment Pending</div>
        ...
        <div style={{ border: '2px solid #6b7280', borderRadius: 12, padding: 12, background: '#f9fafb' }}>
          <div style={{ fontSize: 12, opacity: 0.8, color: '#374151' }}>⏳ Waiting for Designer</div>
        ...
        <div style={{ border: '2px solid #3b82f6', borderRadius: 12, padding: 12, background: '#eff6ff' }}>
          <div style={{ fontSize: 12, opacity: 0.8, color: '#1e40af' }}>🎨 In Progress</div>
        ...
        <div style={{ border: '2px solid #10b981', borderRadius: 12, padding: 12, background: '#d1fae5' }}>
          <div style={{ fontSize: 12, opacity: 0.8, color: '#065f46' }}>✅ Completed</div>
        ...
        <div style={{ border: '2px solid #ef4444', borderRadius: 12, padding: 12, background: '#fef2f2' }}>
          <div style={{ fontSize: 12, opacity: 0.8, color: '#991b1b' }}>❌ Cancelled</div>
        ...
        <div style={{ border: '2px solid #8b5cf6', borderRadius: 12, padding: 12, background: '#f3e8ff' }}>
          <div style={{ fontSize: 12, opacity: 0.8, color: '#5b21b6' }}>💰 Total Spent</div>
      ...
      <div style={{ border: '1px solid var(--border)', borderRadius: 12, padding: 16 }}>
        <h3 style={{ marginTop: 0, marginBottom: 16 }}>📋 Project Progress Tracking</h3>
        {myRequests.length === 0 && (
        ...
        <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'grid', gap: 12 }}>
          {myRequests.map(request => (
            <li key={request.id} style={{ border: '1px solid var(--border)', borderRadius: 8, padding: 12 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                    <strong style={{ fontSize: 16 }}>{request.candidateName}</strong>
                    <span style={{ fontSize: 12, background: getClientStatusColor(request.workStatus, request.paid) + '20', color: getClientStatusColor(request.workStatus, request.paid), padding: '2px 8px', borderRadius: 12, fontWeight: 600 }}>
                      {getClientStatusIcon(request.workStatus, request.paid)} {getClientStatusText(request.workStatus, request.paid)}
                    </span>
                  </div>
                  <div style={{ display: 'grid', gap: 4, marginBottom: 8 }}>
                    <div style={{ fontSize: 13, display: 'flex', alignItems: 'center', gap: 4 }}>
                      <span style={{ opacity: 0.7 }}>🏢</span>
                      <span><strong>Office:</strong> {request.office}</span>
                    </div>
                    <div style={{ fontSize: 13, display: 'flex', alignItems: 'center', gap: 4 }}>
                      <span style={{ opacity: 0.7 }}>💰</span>
                    ...
                    <div style={{ fontSize: 13, display: 'flex', alignItems: 'center', gap: 4 }}>
                      <span style={{ opacity: 0.7 }}>📅</span>
                      <span><strong>Deadline:</strong> {request.deadline || 'N/A'}</span>
                    </div>
                  </div>
                  <div style={{ fontSize: 12, opacity: 0.8, background: '#f3f4f6', padding: 8, borderRadius: 6, borderLeft: '3px solid ' + getClientStatusColor(request.workStatus, request.paid) }}>
                    <strong>Brief:</strong> {request.requirements.slice(0, 150)}{request.requirements.length > 150 ? '...' : ''}
                  </div>
                  {request.submittedWork && request.submittedWork.length > 0 && (
                    <div style={{ fontSize: 12, opacity: 0.8, marginTop: 4 }}>
                      <strong>Submitted Work:</strong> {request.submittedWork.length} file(s) submitted on {new Date(request.completedAt || '').toLocaleDateString()}
                    </div>
                  )}
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8, alignItems: 'flex-end' }}>
                  {request.workStatus === 'completed' && !request.clientRating && (
                    <button
                  ...
                  {request.acceptedDesignerId && (
                    <button
                      onClick={() => setShowMessaging(request.id)}
                      className="button"
                      style={{ fontSize: 12, padding: '4px 8px', background: 'var(--bg-elev)', color: 'var(--muted)', border: '1px solid var(--border)' }}
                    >
                      💬 Message Designer
                    </button>
                  )}
              ...
              {showReviewForm === request.id && (
                <ReviewForm
                  ...
                  revieweeEmail={request.acceptedDesignerId || ''}
                  revieweeName="Designer"
                  type="client_to_designer"
                  onSubmit={handleAddReview}
                  onCancel={() => setShowReviewForm(null)}
                />
              )}
              {showMessaging === request.id && request.acceptedDesignerId && (
                <MessagingPanel
                  currentUser={{ email: userEmail, name: userEmail.split('@')[0] }}
                  otherUser={{ email: request.acceptedDesignerId, name: 'Designer' }}
                  messages={getConversationMessages(request.id)}
                  onSendMessage={(content) => handleSendMessage(content, request.acceptedDesignerId!)}
      ...
      {newMessageToast && (
        <ToastNotification
          message={newMessageToast.message}
...
function DesignerDashboard({ requests, designers, designerEmail, onUpdateRequest, onSendMessage, messages }: { requests: DesignRequest[]; designers: Designer[]; designerEmail: string; onUpdateRequest: (request: DesignRequest) => void; onSendMessage: (message: Omit<Message, 'id' | 'createdAt' | 'read'>) => void; messages: Message[] }) {
  ...
  const getConversationMessages = (requestId: string) => {
    ...
    if (!request) return []
    return messages.filter(m =>
      (m.senderEmail === designerEmail && m.receiverEmail === request.contactEmail) ||
      (m.senderEmail === request.contactEmail && m.receiverEmail === designerEmail)
    )
  }
  ...
  useEffect(() => {
    const unreadMessages = messages.filter(m =>
      m.receiverEmail === designerEmail && !m.read && m.senderEmail !== designerEmail
    )
  ...
  }, [messages, designerEmail, requests, newMessageToast])

  function handleSendMessage(content: string, clientEmail: string) {
    onSendMessage({
      requestId: '',
      senderEmail: designerEmail,
      senderName: currentDesigner?.name || designerEmail.split('@')[0],
  ...
  function acceptWork(requestId: string) {
    ...
    if (request && currentDesigner) {
      ...
      if (confirmAccept) {
        ...
        alert('✅ Work accepted successfully! You can now start working on this project.')
      }
    }
  }
  ...
  return (
    <div className="designer-dashboard" style={{ display: 'grid', gap: 16 }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 12 }}>
        ...
        <div style={{ border: '1px solid var(--border)', borderRadius: 12, padding: 12 }}>
          <div style={{ fontSize: 12, opacity: 0.7 }}>Potential earnings (90% of budget)</div>
          <div style={{ fontSize: 24, fontWeight: 700 }}>{myAcceptedRequests.reduce((sum, r) => sum + calculateDesignerEarnings(r.budget), 0) > 0 ? formatKES(myAcceptedRequests.reduce((sum, r) => sum + calculateDesignerEarnings(r.budget), 0)) : 'N/A'}</div>
        </div>
      </div>

      <div style={{ border: '2px solid #10b981', borderRadius: 12, padding: 16, background: '#10b98105' }}>
        <h3 style={{ marginTop: 0, color: '#10b981' }}>🎨 Work available to accept</h3>
          ...
          {myAcceptedRequests.slice(0, 6).map(r => (
            <li key={r.id} style={{ border: '1px solid #646cff', borderRadius: 8, padding: 8 }}>
              ...
              {r.submittedWork && r.submittedWork.length > 0 && (
                <div style={{ fontSize: 12, opacity: 0.8, marginTop: 4 }}>
                  📁 {r.submittedWork.length} file(s) submitted on {new Date(r.completedAt || '').toLocaleDateString()}
                </div>
              )}
            </li>
          ))}
      ...
      {showMessaging && (
        <MessagingPanel
          currentUser={{ email: designerEmail, name: currentDesigner?.name || designerEmail.split('@')[0] }}
          otherUser={{ email: requests.find(r => r.id === showMessaging)?.contactEmail || '', name: requests.find(r => r.id === showMessaging)?.candidateName || 'Client' }}
          messages={getConversationMessages(showMessaging)}
          onSendMessage={(content) => {
            const request = requests.find(r => r.id === showMessaging)
            if (request) {
              handleSendMessage(content, request.contactEmail)
            }
          }}
          onClose={() => setShowMessaging(null)}
        />
  ...
  return (
    <div className="admin-dashboard" style={{ display: 'grid', gap: 16 }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 12 }}>
        ...
        <div style={{ border: '1px solid var(--border)', borderRadius: 12, padding: 12 }}>
          <div style={{ fontSize: 12, opacity: 0.7 }}>Your Revenue (10% commission)</div>
      ...
      <div style={{ border: '1px solid var(--border)', borderRadius: 12, padding: 12 }}>
        ...
        <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'grid', gap: 8 }}>
          {paid.slice(0, 6).map(r => (
            <li key={r.id} style={{ border: '1px solid var(--border)', borderRadius: 8, padding: 8 }}>
              <strong>{r.candidateName}</strong> — {r.office}
              <div style={{ fontSize: 12, opacity: 0.8 }}>Budget: {r.budget || 'N/A'} • Fee: {formatKES(r.feeKES || determineFeeKES(r.budget))} • Deadline: {r.deadline || 'N/A'}</div>
              <div style={{ fontSize: 12, opacity: 0.8 }}>Contact: {r.contactEmail}</div>
            </li>
          ))}
        </ul>
      ...
      <div style={{ display: 'grid', gap: 8 }}>
        {filteredRequests.map(r => (
          <div key={r.id} style={{ border: '1px solid var(--border)', borderRadius: 12, padding: 12 }}>
            <div><strong>{r.candidateName}</strong> — {r.office} ({r.paid ? 'Paid' : 'Unpaid'})</div>
            <div>Budget: {r.budget} • Deadline: {r.deadline}</div>
        ...
        <div style={{ display: 'grid', gap: 8, marginTop: 8 }}>
          {designers.map(d => (
            <div key={d.id} style={{ border: '1px solid var(--border)', borderRadius: 12, padding: 12 }}>
              <div><strong>{d.name}</strong> — {d.location}</div>
...
export default function App() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      const currentMessages = loadFromStorage<Message[]>(STORAGE_KEYS.messages, [])
      const currentNotifications = loadFromStorage<Notification[]>(STORAGE_KEYS.notifications, [])
  ...
  function sendMessage(message: Omit<Message, 'id' | 'createdAt' | 'read'>) {
    ...
    const suspiciousPatterns = [
      ...
      /\b07\d{8}\b/g, // Kenyan phone numbers starting with 07
      /\b254\d{9}\b/g, // Kenyan numbers with country code
      /\+254\d{9}\b/g, // Kenyan numbers with +
      /whatsapp\.com|wa\.me|telegram|signal|viber/gi, // External messaging apps
      /pay\s*me|send\s*money|cash|mpesa|bank\s*account/gi, // Payment requests
      /outside|off\s*platform|direct\s*payment|skip\s*platform/gi, // Platform bypass attempts
      /email\s*me|gmail\.com|yahoo\.com|hotmail\.com/gi, // Email exchanges
    ]
    ...
    const newMessage: Message = {
      ...
      createdAt: new Date().toISOString(),
      read: false,
      flagged: hasSuspiciousContent
    }

    // Update local state immediately
    setMessages(prev => [newMessage, ...prev])

    // Save to localStorage immediately for cross-window communication
    const updatedMessages = [newMessage, ...messages]
    saveToStorage(STORAGE_KEYS.messages, updatedMessages)

    // Alert admin if suspicious content detected
    if (hasSuspiciousContent) {
      const adminNotification: Notification = {
        id: crypto.randomUUID(),
        userEmail: 'admin@campaignbridge.com',
        title: '🚨 Suspicious Message Detected',
        message: `Suspicious content in message between ${message.senderName} (${message.senderEmail}) and ${message.receiverEmail}: "${message.content.substring(0, 100)}..."`,
        type: 'warning',
      ...
      saveToStorage(STORAGE_KEYS.notifications, [adminNotification, ...notifications])
    }
    ...
    setNotifications(prev => [receiverNotification, ...prev])
    saveToStorage(STORAGE_KEYS.notifications, [receiverNotification, ...notifications])
  }
        ...
        <main className="main-content" style={{ display: 'grid', gap: 16 }}>
          {view === 'dashboard' && (
            <section>
                ...
                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                  {userRole && (
                    <span style={{ fontSize: 12, opacity: 0.8, padding: '4px 8px', background: 'var(--bg-elev)', borderRadius: 4 }}>
                      {userRole === 'admin' ? '👑 Admin' : userRole === 'client' ? '👤 Client' : '🎨 Designer'} ({userEmail})
                    </span>
                  )}
                  {!userRole && (
              ...
              {userRole === 'client' && <ClientDashboard requests={requests} userEmail={userEmail} onAddReview={addReview} onSendMessage={sendMessage} messages={messages} />}
              {userRole === 'designer' && <DesignerDashboard requests={requests} designers={designers} designerEmail={userEmail} onUpdateRequest={updateRequest} onSendMessage={sendMessage} messages={messages} />}
          ...
          {view === 'request' && (
            <section>
              ...
              <p style={{ marginTop: -6, fontSize: 13, opacity: 0.8 }}>A small posting fee in KES (400–800) applies based on your project budget.</p>
